//! Vendored Qwen2 / Qwen2.5 adapter with explicit KV-cache access.
//!
//! Adapted from `candle-transformers` 0.10.2. Covers Qwen2, Qwen2.5, QwQ,
//! and Qwen2.5 Coder variants.
...
impl Qwen2Attention {
    fn new(cfg: &Config, rotary_emb: Arc<Qwen2RotaryEmbedding>, vb: VarBuilder) -> Result<Self> {
...
impl DecoderLayer {
    fn new(cfg: &Config, rotary: Arc<Qwen2RotaryEmbedding>, vb: VarBuilder) -> Result<Self> {
        let self_attn = Qwen2Attention::new(cfg, rotary, vb.pp("self_attn"))?;
...
impl Model {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let rotary = Arc::new(Qwen2RotaryEmbedding::new(vb.dtype(), cfg, vb.device())?);
