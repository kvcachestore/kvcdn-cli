//! Vendored GLM4 adapter with explicit KV-cache access.
//!
//! Adapted from `candle-transformers` 0.10.2 `models/glm4.rs`. Supports the
//! GLM4 9B chat / base checkpoints using the classic ChatGLM-style config
//! (`padded_vocab_size`, `multi_query_attention`, custom RoPE, etc.).
...
impl SelfAttention {
    fn new(layer_number: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if !cfg.multi_query_attention {
            candle_core::bail!("GLM4 adapter requires multi_query_attention=true")
        }
...
impl Block {
    fn new(layer_number: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if !cfg.rmsnorm {
            candle_core::bail!("GLM4 adapter requires rmsnorm=true")
        }
...
impl Transformer {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let final_layernorm = if cfg.post_layer_norm {
            ...
            candle_core::bail!("GLM4 adapter requires post_layer_norm=true")
        };
