//! Vendored ChatGLM2 / ChatGLM3 adapter with explicit KV-cache access.
//!
//! Adapted from `candle-transformers` 0.10.2 `models/chatglm.rs`. Covers the
//! classic ChatGLM-style config (`padded_vocab_size`, `multi_query_attention`,
//! custom RoPE, RMSNorm, etc.).
...
impl RotaryEmbedding {
    ...
    fn apply(&self, xs: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        let (seqlen, _b, np, head_dim) = xs.dims4()?;
        ...
        if rot_dim < head_dim {
            let xs_pass = xs.narrow(D::Minus1, rot_dim, head_dim - rot_dim)?;
...
impl SelfAttention {
    fn new(layer_number: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if !cfg.multi_query_attention {
            candle_core::bail!("ChatGLM adapter requires multi_query_attention=true")
        }
        ...
        let query_key_value = if cfg.add_bias_linear || cfg.add_qkv_bias {
            linear(cfg.hidden_size, qkv_hidden_size, vb.pp("query_key_value"))?
        } else {
            linear_no_bias(cfg.hidden_size, qkv_hidden_size, vb.pp("query_key_value"))?
        };
        let core_attention = CoreAttention::new(layer_number, cfg, vb.dtype())?;
        let dense = if cfg.add_bias_linear {
            linear(cfg.hidden_size, cfg.hidden_size, vb.pp("dense"))?
        } else {
            linear_no_bias(cfg.hidden_size, cfg.hidden_size, vb.pp("dense"))?
        };
        Ok(Self {
            query_key_value,
            core_attention,
    ...
    fn forward(
        ...
        let query_layer = rotary_emb.apply(&query_layer, offset)?;
        let key_layer = rotary_emb.apply(&key_layer, offset)?;
        ...
        context_layer.apply(&self.dense)
    }
}
...
impl MLP {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let dense_h_to_4h = if cfg.add_bias_linear {
            linear(
                cfg.hidden_size,
        ...
        let dense_4h_to_h = if cfg.add_bias_linear {
            linear(cfg.ffn_hidden_size, cfg.hidden_size, vb.pp("dense_4h_to_h"))?
        } else {
            linear_no_bias(cfg.ffn_hidden_size, cfg.hidden_size, vb.pp("dense_4h_to_h"))?
        };
        Ok(Self {
            dense_4h_to_h,
            dense_h_to_4h,
...
impl Block {
    fn new(layer_number: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if !cfg.rmsnorm {
            candle_core::bail!("ChatGLM adapter requires rmsnorm=true")
        }
    ...
    fn forward(
        ...
        let attention_output =
            self.self_attention
                .forward(&layernorm_output, attention_mask, rotary_emb, offset)?;
...
impl Transformer {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        if !cfg.post_layer_norm {
            candle_core::bail!("ChatGLM adapter requires post_layer_norm=true")
        }
        let final_layernorm = candle_nn::rms_norm(
            cfg.hidden_size,
            cfg.layernorm_epsilon,
    ...
    fn forward(
        ...
        let mut xs = xs.clone();
        for block in self.layers.iter_mut() {
            xs = block.forward(&xs, attention_mask, &self.rotary_emb, offset)?
        }
...
fn causal_mask(size: usize, offset: usize, device: &Device) -> Result<Tensor> {
    let mask: Vec<_> = (0..size)
        .flat_map(|i| (0..(size + offset)).map(move |j| u8::from(j > i + offset)))
        .collect();
    Tensor::from_slice(&mask, (size, size + offset), device)
}
...
impl Model {
    ...
    fn set_kv_cache(&mut self, cache: KVCache) -> Result<()> {
        if cache.len() != self.encoder.layers.len() {
            candle_core::bail!(
        ...
        for (layer, (k, v)) in self.encoder.layers.iter_mut().zip(cache) {
    ...
    pub fn forward(&mut self, input: &Tensor, offset: usize) -> Result<Tensor> {
        let (_b_size, seq_len) = input.dims2()?;
        let input_embeds = self.embedding.forward(input)?;
        let attention_mask = if seq_len <= 1 {
            None
        } else {
            Some(causal_mask(seq_len, offset, input_embeds.device())?)
        };
        self.encoder.forward(&input_embeds, &attention_mask, offset)
    }
}
...
impl ModelForCausalLM {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let output_layer = linear_no_bias(
            cfg.hidden_size,
            cfg.padded_vocab_size,
            vb.pp("transformer").pp("output_layer"),
        )?;
        Ok(Self { base, output_layer })
    }

    pub fn forward(&mut self, input: &Tensor, offset: usize) -> Result<Tensor> {
        let (_, seq_len) = input.dims2()?;
        let hidden = self.base.forward(input, offset)?; // (seq_len, b, h)
        let last = hidden.i(seq_len - 1)?; // (b, h)
        let logits = last.apply(&self.output_layer)?; // (b, vocab)
        logits.unsqueeze(1) // (b, 1, vocab)
    }
...
fn to_model_config(cfg: &Config) -> ModelConfig {
    ModelConfig {
        ...
        num_key_value_heads: cfg.multi_query_group_num,
    }
}
